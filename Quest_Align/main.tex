...
conducted with 10~participants (2~female, 8~male; ages 22--25) performing a seven-step,
35~s-per-participant posture protocol (baseline, forward flexion, two lateral inclinations,
and three wall-supported recovery segments), yielding $N=2{,}454$ observer-labeled
frame-level samples recorded at a mean effective synchronized-pair frequency of 5.0~Hz
(range 2.7--6.7~Hz), well below the 60~Hz rate used to train the HMD-Poser checkpoint. The
resulting posture classification achieved 65.77\% accuracy, a macro-averaged F1-score of
0.6739, and Cohen's~$\kappa$ of 0.4202. These results demonstrate operational feasibility
but not biomechanical validity, because no synchronized optical motion-capture,
inclinometer, or radiographic reference was collected, and the pelvis-only sensor
configuration was not among HMD-Poser's original training configurations.}
\end{abstract}
...
summarizing the controlled-protocol evaluation over \CC{$N=2{,}454$} labeled frame-level samples.}
...
and 2,454 observer-labeled frame-level samples, used to characterize acquisition consistency
...
quaternion and acceleration vector measured near the pelvis. This additional channel
...
\CC{Table~\ref{tab:protocol_ieee} presents the protocol in tabular form. Unlike the
75~s/step protocol used in earlier prototype iterations, each step was shortened to a
uniform 5~s in the version used for the reported evaluation, giving a nominal duration of
35~s per participant. The 5~s-per-step design was chosen to better match the system's
effective acquisition rate and to leave headroom in a session for reconnection delays. Because
of these delays and posture-transition overhead, the mean effective duration across the
10~recorded sessions was 47.7~s (range 35.5--60.6~s), i.e., systematically longer than the
35~s nominal target, yielding a total of 477.3~s of effective acquisition across all
participants.}
...
\CC{\subsection{Participants}
Ten participants (2~female, 8~male; ages 22--25, height 1.56--1.75~m, weight
53.0--69.0~kg) completed the protocol in May~2026. Table~\ref{tab:participants} summarizes
individual anthropometrics. No participant reported a diagnosed spinal condition prior to
the session; this was not independently verified through medical records.

\begin{table}[t]
\caption{Participant anthropometrics ($N=10$, May 2026)}
...
\begin{tabular}{ccccc}
\toprule
Session & Gender & Age & Height (m) & Weight (kg) \\
\midrule
1  & F & 24 & 1.56 & 55.0 \\
2  & M & 23 & 1.67 & 69.0 \\
3  & M & 22 & 1.73 & 63.0 \\
...
The Android orientation sensor was verified as \texttt{TYPE\_ACCELEROMETER} (gravity
included). After transforming the raw acceleration into the internal right-handed
coordinate frame (X~right, Y~up, Z~forward, meters, m/s$^2$) via the phone-to-pelvis
calibration matrix $\mathbf{R}_{\text{internal from android}}$ -- computed during a
mandatory neutral-pose calibration step -- the server subtracts the internal gravity vector
$[0,\,-9.80665,\,0]$~m/s$^2$.}
...
frame was repeated to fill the model's temporal window. Because the effective
synchronized-pair frequency measured across the ten experimental sessions averaged 5.0~Hz
(range 2.7--6.7~Hz), the resulting 40-frame window spans approximately 8~s of real time,
compared with the 0.67~s window implied by the 60~Hz frame rate used during HMD-Poser
...
Table~\ref{tab:protocol_ieee}. The nominal duration was 35~s per participant; the mean
...
2{,}454~individually timestamped frames. This paper reports results over the full recorded
protocol, $N=2{,}454$.}
...
\end{equation}
\CC{Across the ten sessions, the mean recorded synchronized-pair rate was 5.0~Hz, with
session-level values ranging from 2.7 to 6.7~Hz. Therefore, the nominal 60~Hz server-loop
frequency should not be interpreted as the effective sensor-acquisition, synchronization,
inference, or valid-output rate. An earlier 18-session dataset, collected with the initial
(single-frame-repeat) prototype pipeline and reporting a mean rate of 7.5~Hz, is superseded
by this ten-participant dataset and is not used in the analysis below.}
...
seven-step protocol of Table~\ref{tab:protocol_ieee}, with the three wall-supported
...
Sections~IV-D through IV-F. All 2,454~frames come from the synchronized HMD/controller-plus-smartphone
...
\begin{figure}[H]
    ...
    Align predictions over $N=2{,}454$ frame-level samples from the ten-participant,
...
65.77\%, with 1{,}614~correctly classified frames out of 2{,}454~samples. The Center class
produced 918~true positives, 305~false positives, 711~true negatives, and 520~false
negatives. The Lateral class produced 441~true positives, 523~false positives,
1{,}304~true negatives, and 186~false negatives. The Forward class produced 255~true
positives, 12~false positives, 2{,}053~true negatives, and 134~false negatives. The main
error pattern is between Center and Lateral: 514~true Center frames were predicted as
Lateral, and 180~true Lateral frames were predicted as Center. In addition, 125~true
Forward frames were predicted as Center (and only 9 as Lateral), suggesting that
forward-flexion events were more often confused with a return to center than with lateral
deviation, consistent with the predicted trunk pitch not consistently exceeding the alert
threshold during transitional frames.}

\begin{table}[t]
\caption{\CC{One-vs-rest counts and classification metrics for the controlled posture-orientation test ($N=2{,}454$)}}
...
\midrule
Center  & 918 & 305 & 711  & 520 & 75.1\% & 63.8\% & 0.6900 & 70.0\% \\
Lateral & 441 & 523 & 1304 & 186 & 45.8\% & 70.3\% & 0.5544 & 71.4\% \\
Forward & 255 & 12  & 2053 & 134 & 95.5\% & 65.5\% & 0.7774 & 99.4\% \\
\midrule
Macro avg. & -- & -- & -- & -- & 72.1\% & 66.6\% & 0.6739 & 80.3\% \\
\bottomrule
\end{tabular}}
\end{table}

\CC{The metric summary in Table~\ref{tab:posture_metrics} indicates that the model remains
most conservative for the Forward class: it achieves the highest precision (95.5\%) and
specificity (99.4\%), but a lower recall (65.5\%), meaning that some forward-flexion frames
are not detected as such. The Lateral class achieves the highest recall (70.3\%) but the
lowest precision (45.8\%), indicating that lateral deviation is over-predicted, especially
during Center frames. The Center class shows intermediate but still limited performance,
with 75.1\% precision, 63.8\% recall, and an F1-score of 0.6900. At the aggregate level, the
macro-averaged F1-score is 0.6739 and Cohen's~$\kappa$ is 0.4202, indicating moderate
agreement between observer labels and Quest Align predictions -- essentially unchanged in
overall magnitude relative to earlier prototype iterations, despite the substantial
methodological corrections to the feature layout, temporal window, and forward-kinematics
extraction described in Sections~IV-D through IV-F. This suggests that the previously
reported classification performance was not primarily an artifact of the single-frame-repeat
or generic-padding pipeline, and that the remaining error is more likely attributable to the
low effective sensor frequency, the pelvis-only sensor configuration (untested during
HMD-Poser training), and sensor-to-segment calibration of the pelvis-mounted smartphone.
These limitations are consistent with the observed IMU disconnections and with the
sensitivity of the posture estimate to smartphone placement and wireless stability.}
...
be interpreted as 2,454 statistically independent, balanced observations. Moreover,
...
\CC{The ten-session evaluation of the refactored, 135-dimensional pipeline yielded a mean
synchronized-pair frequency of 5.0~Hz and a session-level range of 2.7--6.7~Hz,
substantially below the nominal 60~Hz server-loop target and somewhat lower than the 7.5~Hz
mean previously measured on the superseded single-frame-repeat pipeline. Because the model
now receives a genuine 40-frame window rather than a repeated frame, this low frequency
translates directly into a temporal-horizon mismatch: the window spans roughly 8~s of real
time versus the 0.67~s window used during HMD-Poser training, and no resampling is applied
to reconcile the two. A future runtime benchmark should separately report sensor,
...
effective duration per session) produced $N=2{,}454$ observer-labeled frame-level samples;
the classification evaluation on the refactored pipeline yielded an overall accuracy of
65.77\%, a macro-averaged F1-score of 0.6739, and a Cohen's~$\kappa$ of 0.4202, indicating
